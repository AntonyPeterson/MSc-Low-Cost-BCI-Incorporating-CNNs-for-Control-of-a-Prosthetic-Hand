...
class SecondScreen(Screen):
	...
	def create_choices(self):
		if self.mi_status:
			classes = ['Foot', 'Left Hand', 'Right Hand', 'Tongue'] #,'Nothing']
		else:
...
class ThirdScreen(Screen):
	...
	def Label_Change(self, dt):
		...
		#self.board.config_board('x1040110Xx2040110Xx3040110Xx4040110Xx5040110Xx6040110Xx7040110Xx8040110X')
		print('Session Ready...')
	...
	def start_stop_eeg(self):
		...
		'''eeg_data = df.iloc[:,1:17]

		specs = np.zeros((16,33,30))
		row = np.zeros((4,33,120))
		for i in range(16):
			spec, freqs, t, im = plt.specgram(eeg_data[i+1], Fs = 256, NFFT = 64, noverlap=32)
			specs[i] = spec
		for i in range(4):
			n= 4*i
			row[i] = np.concatenate((specs[n], specs[n + 1], specs[n + 2], specs[n + 3]), axis = 1)
		combined = np.concatenate((row[0], row[1], row[2], row[3]))
		imageio.imwrite('test' + str(self.trial_start_number+1) +'.png', combined)
		shutil.move('test' + str(self.trial_start_number+1) + '.png', self.participant_name + '\\' + self.taskname + '\\' + str(date.today()))'''
	...
	def startEEGstream(self, dt): 
		...
		if self.random_status:
			...
			#self.taskname = random.choice(['Foot','Right Hand'])
		...
		self.trial_start_number = len(os.listdir(self.participant_name + '\\' + self.taskname + '\\' + str(date.today()))) #/2  #only for specgram
